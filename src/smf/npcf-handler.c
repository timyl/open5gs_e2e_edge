        ...
        OpenAPI_list_for_each(SmPolicyDecision->sess_rules, node) {
            ...
            if (AuthSessAmbr) {
                ...
                if (AuthSessAmbr->downlink) {
                    uint64_t downlink_ambr = ogs_sbi_bitrate_from_string(AuthSessAmbr->downlink);
            ...
            if (AuthDefQos) {
                ...
                sess->session.qos.arp.priority_level = AuthDefQos->priority_level;

                if (AuthDefQos->arp) {
                    if (qos_flow && (qos_flow->qos.arp.priority_level != AuthDefQos->arp->priority_level))
                        session_changed = true;
                    sess->session.qos.arp.priority_level = AuthDefQos->arp->priority_level;

                    if (AuthDefQos->arp->preempt_cap == OpenAPI_preemption_capability_NOT_PREEMPT) {
                        if (qos_flow && (qos_flow->qos.arp.pre_emption_capability != OGS_5GC_PRE_EMPTION_DISABLED))
                            session_changed = true;
                        sess->session.qos.arp.pre_emption_capability = OGS_5GC_PRE_EMPTION_DISABLED;
                    }
                    else if (AuthDefQos->arp->preempt_cap == OpenAPI_preemption_capability_MAY_PREEMPT) {
                        if (qos_flow && (qos_flow->qos.arp.pre_emption_capability != OGS_5GC_PRE_EMPTION_ENABLED))
                            session_changed = true;
                        sess->session.qos.arp.pre_emption_capability = OGS_5GC_PRE_EMPTION_ENABLED;
                    }
                    ogs_assert(sess->session.qos.arp.pre_emption_capability);

                    if (AuthDefQos->arp->preempt_vuln == OpenAPI_preemption_vulnerability_NOT_PREEMPTABLE) {
                        if (qos_flow && (qos_flow->qos.arp.pre_emption_vulnerability != OGS_5GC_PRE_EMPTION_DISABLED))
                            session_changed = true;
                        sess->session.qos.arp.pre_emption_vulnerability = OGS_5GC_PRE_EMPTION_DISABLED;
                    }
                    else if (AuthDefQos->arp->preempt_vuln == OpenAPI_preemption_vulnerability_PREEMPTABLE) {
                        if (qos_flow && (qos_flow->qos.arp.pre_emption_vulnerability != OGS_5GC_PRE_EMPTION_ENABLED))
                            session_changed = true;
                        sess->session.qos.arp.pre_emption_vulnerability = OGS_5GC_PRE_EMPTION_ENABLED;
                    }
        ...
        smf_sess_t *sess, ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    /* 调试 Update default session rules */
