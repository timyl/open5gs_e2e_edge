        ...
        ogs_pfcp_session_establishment_request_t *req)
{
    ...
    for (i = 0; i < OGS_MAX_NUM_OF_QER; i++) {  
        ...
        */
        /*调试1*/
        ogs_pfcp_qer_t *qer = ogs_pfcp_handle_create_qer(&sess->pfcp, &req->create_qer[i],
                                            &cause_value, &offending_ie_value);
        if (qer == NULL)
            break;
        ogs_info("📊 n4-handler create Received QER with MBR Info: UL=%" PRIu64 ", DL=%" PRIu64, qer->mbr.uplink, qer->mbr.downlink);       
        ...
        ogs_pfcp_session_modification_request_t *req)
{
    ...
    for (i = 0; i < OGS_MAX_NUM_OF_QER; i++) {
        ...
        */
        ...
        if (qer == NULL)
            break;
        ogs_info("📊 n4-handler modify Received QER with MBR Info: UL=%" PRIu64 ", DL=%" PRIu64, qer->mbr.uplink, qer->mbr.downlink); 
        ogs_info("📊 n4-handler modify Received QER with GBR Info: UL=%" PRIu64 ", DL=%" PRIu64, qer->gbr.uplink, qer->gbr.downlink);        
        /*结束*/
    ...
    for (i = 0; i < OGS_MAX_NUM_OF_QER; i++) {
        ...
        if (qer == NULL)
            break;
        ogs_info("📊 Received QER with MBR Info: UL=%" PRIu64 ", DL=%" PRIu64, qer->mbr.uplink, qer->mbr.downlink);       
        */
