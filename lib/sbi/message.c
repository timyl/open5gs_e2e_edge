...
ogs_sbi_request_t *ogs_sbi_build_request(ogs_sbi_message_t *message)
{
    ...
    // new added
    if (strcmp(message->h.method, OGS_SBI_HTTP_METHOD_DELETE) == 0) {
        if (message->h.resource.component[3] != NULL &&
            (strcmp(message->h.resource.component[3], OGS_SBI_RESOURCE_NAME_SMF_REGISTRATIONS) == 0)) {
            ogs_sbi_header_set(request->http.headers, OGS_SBI_ACCEPT, OGS_SBI_CONTENT_JSON_TYPE);
            ogs_warn(">>> Overriding Accept header to application/json for DELETE (smf-registrations)");
        } else {
            ogs_sbi_header_set(request->http.headers, OGS_SBI_ACCEPT, OGS_SBI_CONTENT_PROBLEM_TYPE);
        }

    } else if (message->http.accept) {
